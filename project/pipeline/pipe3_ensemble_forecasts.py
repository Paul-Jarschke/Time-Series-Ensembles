...
def pipe3_ensemble_forecasts(individual_predictions, ens_init_train_ratio=0.3, csv_export=False, verbose=False):
    ...
    for i, fc_period in enumerate(range(ens_init_train_size, n_predictions)):
        ...
        indiv_predictions_next = individual_predictions.iloc[fc_period:fc_period+1,]

        current_ensemble_predictions.append(individual_predictions.index[fc_period])

        current_ensemble_predictions.append(float(simple_average(indiv_predictions_next)))
        current_ensemble_predictions.append(float(ensemble_predictions_given_weights(indiv_predictions_next, compute_rmse_weights(current_train))))
        current_ensemble_predictions.append(float(ensemble_predictions_given_weights(indiv_predictions_next, compute_variance_weights(current_train))))
        current_ensemble_predictions.append(float(ensemble_predictions_given_weights(indiv_predictions_next, compute_error_correlation_weights(current_train, verbose=False))))
        current_ensemble_predictions.append(float(metamodel_svr(current_train, indiv_predictions_next)))
        current_ensemble_predictions.append(float(metamodel_random_forest(current_train, indiv_predictions_next)))
