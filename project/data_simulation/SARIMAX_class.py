...
class SARIMAX_Simulation(sm.tsa.SARIMAX):
    def __init__(self, 
                 ...
                 seasonal_order=None, 
                 trend_value=None,
                 ar_coefs=None, ma_coefs=None,
                 sar_coefs=None, sma_coefs=None,
                 shock_sigma2=None,
                 bounded=False,
                 lower_bound=None,
                 upper_bound=None,
        ...
        if nsimulations is not None:
            self.nsimulations = nsimulations
        else:
            self.nsimulations = len(self.months)
        ...
        if len(self.params) != len(self.param_names):  
            raise ValueError(f"Parameters are not correctly defined. There are {len(self.param_names)} parameters but you provided only {len(self.params)}. \
            Check that you have defined all of {self.param_names}.")
    ...
    class SimulatedSeries(pd.Series):
        ...
        def plot(self, title="Simulation", *args, **kwargs):
            ...
            if self.nexog > 0:
                self.plot_title += "X"
            self.plot_title += str(self.order)
            if not (self.seasonal_order[0] == 0 and self.seasonal_order[1] == 0 and self.seasonal_order[2] == 0):
                self.plot_title = "S" + self.plot_title
                self.plot_title += self.seasonal_label
            
            if self.trend:
                self.plot_title += " with trend"
                if self.nexog > 0:
                    self.plot_title += f" and {self.nexog} exogenous variables"
            elif self.nexog > 0:
                    self.plot_title += f" with {self.nexog} exogenous variables"
    ...
    def simulate(self, seed=None):
        ...
        # Simulate series using SARIMAX model
        series = super().simulate(params=self.params, 
                                  nsimulations=self.nsimulations, 
                                  random_state=seed,
                                  measurement_shocks=shocks)
        
        # Adjust series if bounded
        if self.bounded:
            a = self.lower_bound
            b = self.upper_bound
            series = ((series - min(series)) / (max(series) - min(series))) * (b-a)