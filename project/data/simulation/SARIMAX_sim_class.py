...
class SARIMAX_Simulation(sm.tsa.SARIMAX):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.nsimulations = (
            nsimulations if nsimulations is not None else len(self.months)
        )
        ...
        if len(self.params) != len(self.param_names):
            raise ValueError(
                f"Parameters are not correctly defined. Expected {len(self.param_names)} parameters, but provided {len(self.params)}. Check that you have defined all of {self.param_names}."
            )
